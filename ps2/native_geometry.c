...
/*
 * Current VU1 input layout consumes three expanded qwords per vertex before
 * output begins at TOP+96. This is a correctness-era bound, not the final batch
 * size. Larger clusters will be split by the offline compiler or a later VU
 * layout once hardware measurements justify it.
 */
...
/* Shared VU1 matrix columns live outside TOP/TOPS. */
    ...
    const struct CTRPS2NativeGeometryBatch *batch)
{
    ...
    /* First shipping path is textured opaque/static geometry. */
    if (!batch->textured)
        return 0;
...
static void CTRPS2_NativeGeometryBuildHeader(void)
{
    ...
    /* Header 3/4 are reserved for future per-batch constants/material data. */
...
static int CTRPS2_NativeGeometryBuildPacket(void)
{
    ...
    /*
     * CURRENT IMPLEMENTATION correctness fence: packet2 helper emits FLUSH +
     * MSCAL. Once multiple native clusters are flowing, this becomes an A/B
     * target rather than a permanent per-cluster policy.
     */
...
void CTRPS2_NativeGeometrySubmit(void)
{
    ...
    /*
     * CURRENT IMPLEMENTATION: flush=1 covers source-chained REF payloads.
     * Static p2trk streams will later be writebacked at load/finalization so
     * steady-state submission can avoid a broad FlushCache(0).
     */
